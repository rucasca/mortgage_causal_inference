...
Es por ello que a lo largo de este estudio estudiaremos dos tipos de influencia, la relativa al efecto causal del motivo de la solicitud del prestamo sobre la concesión del mismo, y por otro lado el efecto causal de la variable de origen étnico sobre la concesión del prestamo.

Este estudio se llevará a cabo en diferentes notebooks que representan distintas aproximaciones ,siendo esta la primera parte, donde se utilizarán métodos de emparejamiento para recuperar el efecto causal.
...
En segundo lugar, se repite el procedimiento con otras variables. Determinamos que sería interesante estudiar si la raza de la persona es un factor de discriminación para la concesión del préstamo, ya que como se indica en la pagina web donde se encuentra el dataset, una de sus aplicaciones era la detección de factores de discriminación. 
...
Este efecto causal lo obtenemos mediante técnicas como el emparejamiento, que a partir del valor del propensity score nos permite obtener pares de muestras que mantengan distribuciones en X similares. A partir de esta selección de muestras obtenemos el efecto global, sin tener en cuenta posibles efectos heterogéneos del tratamiento.

Se tiene que el efecto global de que el objetivo del prestamo sea refinanciar tiene un efecto sobre la concesión del mismo que indica que las probabilidades de ser rechazado son mayores. Cabe destacar que mediante el método aplicado no somos capaces de recuperar el intervalo de confianza necesario, algo que solucionaremos con los métodos del siguiente apartado.

En cuanto a la discriminación por razas, se tiene que, de nuevo a falta de intervalos de confianza.


En el siguiente notebook, se aplicarán procedimientos que nos permitirán averigar si existen efectos heterogeneos y nos permitirán acotar la influencia de no observables, que no es considerada mediante el método aplicado en esta primera parte.
